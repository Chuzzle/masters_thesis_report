...
\Crefrange{assumption_1c}{assumption_1d} in \cref{assumption_1} implies that the process formed by the event counts for a single class is a Poisson process with rate $W_{\mu}$. Further, we assume that each $W_{\mu}$ is a smooth function of the population. Assumption \ref{assumption_1e} ensures that no more than one event can occur simultaneously. Note also that conditioning the probabilities on the population $X_t$ at $t$ (as we have done) is sufficient, although conditioning based on the event counts $N\left( t \right)$ and the initial population state would also hold in this context. \Crefrange{assumption_1c}{assumption_1d} correspond to assumptions on the marginal distributions of events. Simply put, is is assumed that the probability that a specific event shall occur increases more or less linearly with time (up to a deviation of $o \left( h \right)$.
...
%
%- People can not be immune -and- carriers in this model
